...
class BCRSAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        crowding_response = torch.nn.functional.avg_pool2d(center_response, kernel_size=5, stride=1, padding=2)
        peak_response = torch.nn.functional.max_pool2d(center_response, kernel_size=3, stride=1, padding=1)
        repulsion_response = self.repulsion_gate(
            torch.cat(
                (torch.nn.functional.relu(peak_response - center_response), crowding_response, boundary_response), 1
            )
        )
        ...
        repulsion_cue = self.repulsion_proj(
            projected_contrast_cue * (1.0 + repulsion_response) + edge_boundary_cue * repulsion_response
        )
        if not enable_boundary:
