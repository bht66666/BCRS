...
class BCRSAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        boundary_score = self.boundary_gate(
            torch.cat(
                (
                    edge_mag.mean(1, keepdim=True),
                    laplace_mag.mean(1, keepdim=True),
                    contrast_mag,
                ),
                1,
            )
        ...
        repulsion_score = self.repulsion_gate(
            torch.cat(
                (
                    F.relu(peak_score - center_score),
                    crowd_score,
                    boundary_score,
                ),
                1,
        ...
        if getattr(self, "collect_branch_maps", False):
            self.last_branch_maps = {
                ...
                "center_branch": center_branch.detach().float().cpu(),
                "boundary_branch": boundary_branch.detach().float().cpu(),
                "repulsion_branch": repulsion_branch.detach().float().cpu(),
